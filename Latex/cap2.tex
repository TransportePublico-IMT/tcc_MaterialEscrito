...
\indent
\par Segundo um estudo feito pela UNINDU (The International Congress on University Industry), 83\% das pessoas absorvem melhor as informações através da visão. Isso demonstra a importância do dashboard para tomada de decisões rápidas e melhor análise dos indicadores,melhorando metas e atingindo objetivos \cite{Tecnicon}.

\par O Business Intelligence (BI), por exemplo, é uma área que exige precisão na coleta e controle das informações para gerar insights em base desta ferramenta. Os dados são agrupados em conjuntos de registros e disponibilizadas por meio de dashboards para mensurar o desempenho atual e futuro da empresa de acordo com o cenário \cite{Tecnicon}.
...
\indent
\par A aplicação dos dashboards na gestão empresarial traz muitos benefícios para a tomada de decisão e a visão estratégica do seu negócio \cite{Tecnicon}.

\subsubsection{Auxilia na tomada de decisões}
\indent
\par O processo de tomada de decisões fica cada vez mais fácil através dos dashboards, que centralizam informações de fácil visualização e compreensão, possibilitando uma visão ampla do seu negócio \cite{Tecnicon}.

\subsubsection{Transparência de informações}
\indent
\par Na gestão, é importante que todas as equipes tenham acesso aos indicadores da empresa, mantendo a transparência das informações. As ferramentas de dashboard tem o objetivo de facilitar a comunicação interna entre todos os profissionais \cite{Tecnicon}.

\subsubsection{Otimização de tempo e recursos}
\indent
\par A visualização por dashboards otimiza o tempo para tomarem decisões e evitarem trabalhos manuais e complexos com a organização de dados, passando a priorizar outras atividades mais relevantes \cite{Tecnicon}.
...
\par Por fim, vale ressaltar que o YOLOv3, versão mais nova da ferramenta disponível atualmente, quando comparado com outras ferramentas de mesma finalidade apresenta em média uma precisão parecida na escala mAP-50, porém sua velocidade de processamento é maior, chegando a ser quatro vezes mais rápido, como mostra a figura x. Assim sendo, para análise de imagens em tempo real, nas quais a velocidade é crucial, o YOLO revela-se uma excelente alternativa.
